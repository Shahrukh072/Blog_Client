...
const SearchPage = () => {
  ...
  const searchBlogs = ({page = 1, create_new_arr = false, }) =>{
    ...
    axios.post("http://localhost:4040/search-blogs", {query, page})
    .then( async({ data }) => {
  ...
  const fetchUsers = () =>{
    axios.post("http://localhost:4040/search-users", {query})
    .then(({data: {users}}) =>{
        setUsers( users );
    })
  }
  ...
  return (
    <section className="h-cover flex justify-center gap-10">
      <div className="w-full">
        <InPageNavigation
          routes={[`Search Results form "${query}" `, "Accounts Matched"]}
